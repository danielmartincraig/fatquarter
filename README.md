...
A quilting tool featuring interactive graph paper, multiple drawing tools, and undo/redo. Created with [re-frame](https://github.com/day8/re-frame). Check it out at [danielmartincraig.net](www.danielmartincraig.net).
...
* [`src/cljs/fat_quarter/`](src/cljs/fat_quarter/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/fat_quarter/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `fat-quarter` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/fat_quarter/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns fat-quarter.example
  (:require [fat-quarter.config :as config])
